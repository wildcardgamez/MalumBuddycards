...
public class MalumNullBuddycardPackItem extends BuddycardSetPackItem {
    public MalumNullBuddycardPackItem() {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (level instanceof ServerLevel serverLevel) {
            ...
            double nullLuck = player.getAttribute(RegistryHandler.NULL_LUCK).getValue();
            ...
            for (int i = 0; i < cardAmt; i++) {
                BuddycardItem card = level.getRandom().nextFloat() < nullLuck ? rollNullCard(level.getRandom(), cardLuck) : rollCard(level.getRandom(), cardLuck);
    ...
    public BuddycardItem rollNullCard(RandomSource random, double luck) {
        ...
        return optional
                .map(this::getPossibleCardsWithNull)
    ...
    public List<BuddycardItem> getPossibleCardsWithNull(Rarity rarity) {
        return SET.getCards()
                .stream()
